...
export function ScrollStack({
  ...
  const lastTransformsRef = useRef<Map<number, Record<string, number>>>(new Map());
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      const lenis = new Lenis({
        wrapper: scroller as unknown as Element,
        content: scroller.querySelector(".scroll-stack-inner") as unknown as Element,
        duration: 1.2,
        easing: (t: number) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
        smoothWheel: true,
        touchMultiplier: 2,
